...
public class LevelEditorManager : MonoBehaviour
{
    ...
    public LevelData levelData; // Tham chiếu đến ScriptableObject để lưu trữ dữ liệu level
    ...
    public void LoadRawFile()
    {
        ...
        VoxelData data = JsonUtility.FromJson<VoxelData>(rawJsonFile.text);
    ...
    public void LoadLevelFromScriptableObject()
    {
        if (levelData == null || levelData.voxelData == null || levelData.voxelData.voxels == null)
        {
            Debug.LogWarning("[LevelEditor] Không có ScriptableObject LevelData hoặc dữ liệu voxel để tải.");
            ClearAll();
            return;
        }

        ClearAll();
        VoxelData data = levelData.voxelData;
        ...
        Debug.Log($"[LevelEditor] Đã tải level từ ScriptableObject: {levelData.name}");
    }
    ...
    public void SaveLevelToScriptableObject()
    {
        if (levelData == null)
        {
            Debug.LogError("[LevelEditor] Không có ScriptableObject LevelData để lưu.");
            return;
        }

        if (voxelMap.Count == 0)
        {
            levelData.voxelData = new VoxelData(); // Clear existing data if no voxels
            levelData.gridData = new GridLevelData(); // Clear existing grid data as well
            UnityEditor.EditorUtility.SetDirty(levelData);
            UnityEditor.AssetDatabase.SaveAssets();
            Debug.Log("[LevelEditor] Không có voxel để lưu. Đã xóa dữ liệu trong ScriptableObject.");
            return;
        }

        // Dictionary<string, int> colorStats = new Dictionary<string, int>();
        List<VoxelJsonData> voxelsToSave = new List<VoxelJsonData>(voxelMap.Count);

        List<ColorCount> colorCountList = new List<ColorCount>();
        ...
        foreach (var node in voxelMap.Values)
        {
            ...
            ColorCount existingColorCount = colorCountList.Find(c => c.color == hex);
            if (existingColorCount != null)
            {
                existingColorCount.count++;
            }
            else
            {
                colorCountList.Add(new ColorCount { color = hex, count = 1 });
            }

            voxelsToSave.Add(new VoxelJsonData
            {
                x = node.pos.x,
                y = node.pos.y,
                z = node.pos.z,
                color = hex,
                type = node.type, // Sử dụng loại voxel từ node
            });
        }

        levelData.voxelData = new VoxelData
        {
            size = new VolumeSize { x = gridSize.x, y = gridSize.y, z = gridSize.z },
            voxels = voxelsToSave,
            mark = levelMark,
            colorCountList = new List<ColorCount>(colorCountList)
        };

        UnityEditor.EditorUtility.SetDirty(levelData);
        UnityEditor.AssetDatabase.SaveAssets();
        Debug.Log($"[LevelEditor] Đã lưu level vào ScriptableObject: {levelData.name}");
    }
    ...
    public void PaintVoxelWithUndo(VoxelNode node, Color newColor)
    {
        ...
        if (node.type == VoxelType.Stone)
        {
            Debug.LogWarning("[LevelEditor] Không thể sơn màu lên khối Stone!");
            return;
        }
