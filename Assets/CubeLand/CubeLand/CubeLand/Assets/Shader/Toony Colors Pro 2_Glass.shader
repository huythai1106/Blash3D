Shader "Toony Colors Pro 2/Glass" {
	Properties {
		...
		[TCP2HeaderToggle(TCP2_GLASS_FAKE_BLUR)] _UseFakeBlur ("Fake Background Blur", Float) = 0
		[TCP2ColorNoAlpha] _FakeBlurColor ("Blur Tint", Vector) = (0.86,0.96,1,1)
		_FakeBlurStrength ("Strength", Range(0, 1)) = 0.35
		_FakeBlurCenterStrength ("Center Strength", Range(0, 1)) = 0.25
		_FakeBlurScale ("Scale", Range(1, 64)) = 18
		_FakeBlurContrast ("Contrast", Range(0.25, 4)) = 1.4
		_FakeBlurAlphaBoost ("Alpha Boost", Range(0, 1)) = 0.22
		...
		[HideInInspector] _RenderingMode ("rendering mode", Float) = 2
		[HideInInspector] _SrcBlend ("blending source", Float) = 1
		[HideInInspector] _DstBlend ("blending destination", Float) = 10
